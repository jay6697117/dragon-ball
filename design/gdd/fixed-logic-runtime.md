...
> **Status**: Revised — ninth fresh Design Review MAJOR REVISION blockers addressed; Pending Fresh Re-review
...
> **Design Review**: MAJOR REVISION NEEDED 2026-05-12 on ninth fresh re-review; revised same day to make live-exchange interruptions return through safe neutral, define catch-up preflight as a deterministic classifier with fail-closed uncertainty, add `normal_realtime_critical` presentation tier, separate immutable ack requirements from append-only ack responses, align recovery-pause QA thresholds, declare the MVP profiling device class, and keep ADR gates explicit without creating ADR files; pending fresh re-review.
...
4. **combat tick 不允许静默跳过，catch-up 必须以玩家已看到的关键因果为边界**
   如果浏览器短暂掉帧，运行时最多可以按顺序补跑 `catch_up_max_ticks` 内的少量积压 tick，但只能补跑不会隐藏关键读招因果的 tick。MVP 默认采用小步追赶：`catch_up_max_ticks = 2`，`catch_up_wall_clock_budget_ms = 3.0`，且 catch-up wall-clock 预算包含 preflight descriptor 构建、preflight 分类、模拟、snapshot/event packet 构建、trace metadata 和 delivery metadata。每个 delayed tick 在正式提交前必须先生成只读 `catch_up_preflight_descriptor`，再由 `catch_up_preflight` 分类。MVP `catch_up_preflight` 是确定性只读分类器，不是第二套战斗模拟：descriptor 只能由上一枚已提交 snapshot、目标 tick 已采集/排队的 input snapshot、确定性招式 phase schedule、projectile schedule、已排队 runtime request、staged command set 和只读 runtime metadata 派生；最小字段为 `target_committed_tick_index`、`target_running_tick_index_if_committed`、`preflight_knowledge_source = deterministic_classifier`、`candidate_phase_transitions`、`candidate_projectile_threat_transitions`、`candidate_result_transition_risk`、`candidate_runtime_state_after_commit`、`uses_rng = false`、`descriptor_complete`、`uncertainty_flags` 和 `descriptor_hash`。preflight 不得执行碰撞/伤害/KO 的 shadow resolver，不得修改 runtime state，不得生成 combat event，不得消耗或查询 RNG，不得读取 live Godot node 或表现层状态。若 descriptor 缺失、不完整、hash 不稳定，或 `uncertainty_flags` 显示无法仅凭分类器证明该 delayed tick 不会隐藏 hit/block/whiff/KO、punish/control、projectile threat 或 burst/round-end 因果，必须 fail closed 为 `fairness_stop_required`。运行时不得丢弃 tick、一次性快进大量 tick，或把命中/防御/确反窗口/气弹威胁这类结果藏在不可见补跑里。

   Catch-up fairness 和 presentation delivery 使用四类；`fairness_stop_required` 是 **pre-commit stop classification**，不是已经 emitted 的 combat event tier：

   | Tier | Rule | Minimum events / facts |
   |---|---|---|
   | `fairness_stop_required` | **暂停在结果前并标记 compromised exchange**：如果 delayed tick 的 preflight classifier 显示该 tick 会在玩家未看到原因前进入 active threat、projectile threat、punish opportunity、hit/block/KO/round end 或改变可防御/可反击结果，或分类器无法证明安全，则不得提交该 delayed tick；runtime 停在上一枚已提交 tick，进入 `runtime_state = paused`、`state_reason = recovery_pause`，并记录 `exchange_integrity = compromised_by_runtime_recovery`。正常对战恢复后必须先回到安全中立/重新取得节奏；不得从被中断的确反、命中、防御或控制权窗口中继续结算战术优势。训练模式可以选择重放或延长 telegraph，但 UI 不得泄露比无中断实时战斗更多的战术信息。 | `blocked_transition_type` 至少包括 `active_threat_entered_precommit`、`projectile_threat_entered_precommit`、`punish_window_opened_precommit`、`hit_or_block_result_precommit`、`burst_or_ko_or_round_end_precommit`、`preflight_uncertain_precommit`。这些是未提交 tick 的 guard reason；被 guard 的 event 还没有 emitted。 |
   | `presentation_must_show` | **只 gate 已提交但未被玩家显示水位覆盖的关键因果**：当前 delayed tick 可以提交，但仅当该 fact 由 catch-up/recovery/resume/stale-delivery 风险产生，且会影响玩家理解“为什么被打、为什么挡住、为什么打空、为什么可惩罚、为什么气弹有威胁、为什么控制权返回、为什么 KO/回合结束”时，packet 才生成 `presentation_ack_requirement`。依赖该 fact 的 AI/木桩决策、自动 resume 和新的 silent catch-up tick 必须等待 display watermark、明确视觉降级或 terminal failure policy。audio 不能单独满足 must-show ack。 | 已提交且处于隐藏/压缩/恢复风险中的 `active_frame_started`、`hit_landed`、`blocked`、`whiffed`、`hitstop_started`、`recovery_started`、`punish_window_opened`、`punish_window_closed`、`control_resumed`、`projectile_spawned`、`projectile_threat_changed`、`burst_started`、`ko_started`、`round_ended`。`startup_started` 仅当 catch-up/recovery 会隐藏其起手因果时进入 must-show。 |
   | `normal_realtime_critical` | **必须正常显示但默认不阻塞 runtime**：普通实时 combat 中的 startup、active threat、recovery、punish/control、projectile threat、hit/block/whiff、burst、KO 和 round-end facts 必须进入 packet、event batch、display watermark 顺序和 QA trace；表现层不得当作可丢弃装饰处理。只有当这些 facts 因 catch-up、recovery、resume 或 stale delivery 风险可能被隐藏时，才升级为 `presentation_must_show`。 | 正常实时的 `startup_started`、`active_frame_started`、`recovery_started`、`punish_window_opened`、`punish_window_closed`、`control_resumed`、`projectile_spawned`、`projectile_threat_entered`、`projectile_threat_changed`、`hit_landed`、`blocked`、`whiffed`、`burst_started`、`ko_started`、`round_ended`。 |
   ...
   `presentation_must_show` 事实必须同时携带 `must_show_fact_id`、`minimum_visual_cue_type`、`minimum_visual_cue_requirement`、`required_visual_consumer_policy`、`hud_only_allowed` 和 `minimum_visible_duration_ms`，否则不能生成 satisfied ack。`minimum_visible_duration_ms` 按 fact 类型写入 ack item：normal realtime phase cue 最小 `16.67–50` ms，combat-critical result/threat/punish/control cue 最小 `50–100` ms，非战斗 UI/recovery prompt 最小 `100–250` ms；更长时长由 VFX/HUD/Audio GDD 拥有，但不得低于本范围。MVP 最小 cue map 为：
...
15. **snapshot、event、trace 和 UI state 必须有最小可测合同**  
    ...
    最小 event 必须表达：`runtime_event_sequence_key`、`event_type`、`event_type_ordinal`、`source_actor_id`、`target_actor_id`、`move_id` 或 `cause`、`action_instance_id` when applicable、`hit_instance_id` when applicable、`projectile_instance_id` when applicable、`result_type`、`position_context`、`idempotency_key`、`delivery_context_id`、`presentation_tier` (`presentation_must_show` / `normal_realtime_critical` / `compressible_phase`)、`hud_delta` 或 `hud_pulse_hint` when applicable。`fairness_stop_required` 只允许出现在 pre-commit descriptor、catch-up stop reason 和 delivery context 的 `contains_fairness_stop_preflight`，不得作为已提交 combat event tier。`idempotency_key` 作用域为 `(presentation_generation_id, round_instance_id, runtime_event_sequence_key or runtime_state_sequence_key, event_type, source_actor_id, target_actor_id, move_id_or_cause, action_instance_id, hit_instance_id, projectile_instance_id)`；同一 key 的 HUD pulse、VFX one-shot、audio one-shot、UI one-shot、debug one-shot 在同一 generation 内最多播放一次。
    ...
    `presentation_ack_requirements` 是 immutable packet 内的只读需求记录，不得包含会被消费者回填的 mutable 状态。最小字段为：`presentation_ack_id`、`presentation_generation_id`、`ack_owner`、`required_visual_consumer_set`、`required_visual_consumer_policy` (`any_of` / `all_of` / `specific_consumer_required`)、`optional_audio_consumer_set`、`must_show_fact_ids`、`must_show_fact_ids_hash`、`ack_requirement_items`、`display_watermark_target`、`ack_deadline_policy`、`ack_deadline_ms` 和 `ack_lifecycle_record_id`。每个 `ack_requirement_item` 必须包含 `must_show_fact_id`、`minimum_visual_cue_type`、`minimum_visual_cue_requirement`、`allowed_consumer_types`、`hud_only_allowed`、`spatial_combat_fact`、`non_color_backup_required` 和 `minimum_visible_duration_ms`。`ack_deadline_policy` MVP 允许：`wait_until_ack`、`degrade_after_deadline`、`fail_terminal_after_deadline`、`discard_if_stale_generation`；combat-critical must-show facts 默认使用 `degrade_after_deadline`，并使用 `ack_deadline_ms = combat_critical_ack_deadline_ms = 50` 或 next host-frame boundary 中较早可测结果；非战斗 UI/recovery 提示可使用 `presentation_ack_deadline_ms = 250`。deadline 到期后必须追加独立 ack response / lifecycle record，记录 `ack_response_result = visually_degraded` 并显示等价可读视觉降级提示；如果降级也失败，必须追加 `ack_response_result = failed_terminal`，阻止自动 resume、AI/木桩决策或新的 silent catch-up 继续依赖该 fact，直到 stale discard、safe-neutral recovery、training replay 或 restart policy 明确解决。Display watermark 成立条件：符合 policy 的 visual consumer 确认关键事实已提交到一个可呈现的 host-frame boundary，并且对应 `ack_requirement_item` 的 cue 合同满足；normal-combat spatial facts 必须由 `actor_sprite`、`projectile_vfx` 或 `combat_vfx` 满足，HUD/UI 只能辅助，除非 mode 明确为 training/debug/accessibility assist。该 ack 只证明显示水位和最小 cue 合同达成，不证明玩家真的看见或理解。audio 可以附加 ack，但 audio unlock、静音或浏览器策略导致的 audio-only delivery 不能单独满足 must-show ack。任何依赖 must-show fact 的 CPU/木桩决策、后续 silent catch-up tick 或自动 resume，都必须等待该 ack、降级记录或 terminal failure policy。

    Presentation ack response / lifecycle 必须是独立 append-only 只读记录，不得预先塞进 immutable packet，也不得回写 patch 旧 packet。response 记录最小字段为：`presentation_ack_id`、`presentation_generation_id`、`round_instance_id`、`round_instance_sequence`、`consumer_id`、`consumer_type` (`actor_sprite` / `projectile_vfx` / `combat_vfx` / `hud` / `ui_overlay` / `audio_optional` / `debug_optional` / `runtime_presentation_coordinator`)、`must_show_fact_ids_hash`、`ack_requirement_item_results`、`acknowledged_presented_running_tick_index`、`ack_response_result` (`acknowledged` / `visually_degraded` / `failed_terminal` / `discarded_stale_generation` / `discarded_stale_round` / `discarded_superseded_ack` / `safe_neutral_reacquired`)、`host_frame_index`、`host_time_ms`、`visual_cue_type`、`minimum_visual_cue_met`、`non_color_backup_met`、`ack_deadline_missed_by_host_stall` 和 `stale_policy_result`。`primary_snapshot_consumer` 是 MVP 的 `runtime_presentation_coordinator` 逻辑消费者：它只证明当前 packet 的 snapshot summary 已被原子交付到 HUD/actor sprite/projectile VFX/combat VFX/UI 队列并到达 host-frame boundary，不证明 normal-combat spatial fact 已由 HUD 单独满足。Runtime 只接受当前 round、当前 `presentation_generation_id`、匹配 `presentation_ack_id`、匹配 `must_show_fact_ids_hash`、满足 per-fact cue policy、且 `acknowledged_presented_running_tick_index >= display_watermark_target` 的 response；watermark 实际推进仍必须按下一枚未呈现 running tick 顺序推进，不能因为后续 tick 先 ack 而跳过前序 tick。旧 generation、旧 round、被 supersede 的 ack、错误 target index 和 HUD-only normal-combat spatial ack 必须丢弃并记录 stale/invalid diagnostics。
...
17. **运行时必须支持 bounded QA trace**  
    ...
    MVP 默认 Web performance / memory budgets：`host_frame_p95_budget_ms = 16.67`、`host_frame_p99_budget_ms = 25.0`、`runtime_normal_tick_budget_ms = 1.0`、`catch_up_wall_clock_budget_ms = 3.0`、`max_single_frame_stall_ms = 50.0`、`profiling_heap_growth_fail_bytes = 4194304` over 60 seconds、`profiling_gc_pause_fail_ms = 8.0` when browser/runtime exposes GC pause data、`combat_critical_ack_deadline_ms = 50`、`presentation_ack_deadline_ms = 250`。MVP blocking device class 默认为 `desktop_laptop_integrated_gpu_2020_or_newer`：硬件加速开启、非省电模式、浏览器 DevTools 关闭、清洁浏览器 profile、1280×720 gameplay canvas、系统后台负载保持低噪声；Profiling/QA Instrumentation ADR 可以收紧或替换该 class，但未声明替代 class 前不得把 Web profiling 标记为通过。`combat_critical_ack_deadline_ms` 适用于命中/格挡/打空、active threat、projectile threat、punish/control、burst/KO/round-end 等战斗关键 cue；必须在 next host-frame boundary 或 50ms 内 ack/degrade/fail。如果 host-frame stall 让 next host-frame 晚于 deadline，runtime 必须记录 missed-host-frame diagnostic 并执行 degraded/terminal policy，不能把不可见等待记为成功 ack。`presentation_ack_deadline_ms` 只作为非战斗 UI、恢复提示和调试 overlay 的有界等待上限。`catch_up_attempt_budget_ms` 不再是独立 tuning knob；若 registry 中保留该旧名，只能作为 deprecated alias 指向 `catch_up_wall_clock_budget_ms`。这些预算用于 exported Web profiling 的 QA 起点，不代表最终优化目标；Runtime Profiling ADR 可基于真实浏览器实测调整，但必须解释调整原因并保留自动化或手动证据。Pre-round readiness 是 first-round 阻塞条件：countdown 进入 `go` 前，HUD、actor sprite、projectile/combat VFX、audio unlock/degradation state 和 debug-minimal trace 必须处于 ready 或 explicit degraded-ready；第一下 hit/block、第一枚 projectile、第一次 burst、第一次 HUD pulse 或第一次 quick restart 不得成为 shader/asset/audio 初始化卡顿点。
...
| Fairness stop required | `fairness_stop_required` | bool | `{true, false}` | delayed tick 正式提交前的只读 deterministic classifier 是否发现会隐藏关键读招因果，或因 `uncertainty_flags` 无法证明安全；true 时该 delayed tick 不提交。 |
...
- **If catch-up preflight descriptor is missing, incomplete, unstable, uncertain, or finds a `fairness_stop_required` fact**: 不提交该 delayed tick；runtime 停在上一枚已提交 tick，进入 `runtime_state = paused`、`state_reason = recovery_pause`，trace 记录 `catch_up_stop_reason = fairness_guard`、`blocked_transition_type`、`blocked_tick_index`、`preflight_knowledge_source`、`uncertainty_flags`、`descriptor_complete`、`descriptor_hash` 和 `presentation_priority_reason`，并标记 `exchange_integrity = compromised_by_runtime_recovery`。MVP `fairness_stop_required` 至少包括将进入 active threat 的攻击或 projectile、`punish_window_opened`、`projectile_threat_entered`、以及可能在未展示前导致 `hit_landed`、`blocked`、`whiffed`、`burst_started`、`ko_started`、`round_ended` 或改变可反击/可防御结果的事实；如果 deterministic classifier 无法证明这些不会发生，也必须按 `preflight_uncertain_precommit` fail closed。
...
| `combat_critical_ack_deadline_ms` | next host-frame boundary, hard-capped at `50` ms | `16.67–100` ms | hit/block/whiff、active/projectile threat、punish/control、burst/KO/round-end 等战斗关键 cue 的 ack/degrade/fail 上限；host-frame stall 超过 deadline 时必须记录 missed-host-frame diagnostic 并降级或 terminal fail。 | 太低会把浏览器正常 host-frame jitter 误判为失败。 | 太高会制造玩家能感知的微暂停或晚提示，破坏读招。 | Presentation Delivery ADR 可调整；默认按玩家优先保守策略。 |
...
**Given** delayed tick `T` has a `catch_up_preflight_descriptor` whose deterministic classifier reports active threat, punish window, projectile threat, hit/block/whiff/KO/round-end risk, `descriptor_complete = false`, unstable `descriptor_hash`, or non-empty `uncertainty_flags`, **When** `catch_up_preflight` evaluates tick `T`, **Then** tick `T` is not committed during silent catch-up, runtime remains at the previous committed tick, runtime enters `runtime_state = paused` with `state_reason = recovery_pause`, and trace records `catch_up_stop_reason = fairness_guard`, `blocked_tick_index = T`, `blocked_transition_type`, `preflight_knowledge_source = deterministic_classifier`, `uncertainty_flags`, `descriptor_complete`, `descriptor_hash`, `presentation_priority_reason`, and `exchange_integrity = compromised_by_runtime_recovery`.
...
**Given** QA validates event presentation tiering, **When** an event is a normal realtime startup/threat/result/punish/control/projectile/burst/KO/round-end fact, **Then** its `presentation_tier` is `normal_realtime_critical` unless catch-up/recovery/resume/stale-delivery risk upgrades it to `presentation_must_show`; non-critical repeated idle/spacing/debug facts may use `compressible_phase`; `fairness_stop_required` never appears as an emitted event tier.
...
**Given** a `presentation_delivery` or `bound_diagnostic` record is emitted, **When** QA validates the trace schema, **Then** delivery records include `presentation_generation_id`, `packet_sequence_key`, `delivery_context_id`, `delivery_mode`, `delivery_tick_start`, `delivery_tick_end`, `final_snapshot_committed_tick_index`, `presentation_ack_required`, `presentation_ack_id`, `display_watermark_target`, `latest_ack_response_result`, and `stale_policy_result`; bound diagnostics include `bound_exceeded_type`, `configured_limit`, `observed_value`, and `entries_evicted_or_throttled`.
...
**Given** match setup loads a first-round/simple CPU profile, **When** runtime validates profile metadata, **Then** startup trace exposes `ai_fairness_profile_id`, `configured_min_decision_age_ticks`, and `first_round_profile`, and exported gameplay rejects the profile before countdown if `configured_min_decision_age_ticks < 12`.
...
**Given** catch-up preflight detects a `fairness_stop_required` fact or cannot prove delayed tick `T` safe with the deterministic classifier, **When** the runtime handles the backlog, **Then** tick `T` is not committed during silent catch-up, the runtime remains at the previous committed tick, enters `runtime_state = paused` with `state_reason = recovery_pause`, and records `catch_up_stop_reason = fairness_guard` with `blocked_tick_index = T`, `preflight_knowledge_source = deterministic_classifier`, and `uncertainty_flags` when present.
...
**Given** the packet contains `presentation_ack_requirements`, **When** QA validates the packet, **Then** each immutable requirement includes `presentation_ack_id`, `presentation_generation_id`, `ack_owner`, `required_visual_consumer_set`, `required_visual_consumer_policy`, `optional_audio_consumer_set`, `must_show_fact_ids`, `must_show_fact_ids_hash`, `ack_requirement_items`, `display_watermark_target`, `ack_deadline_policy`, `ack_deadline_ms`, and `ack_lifecycle_record_id`, and does not contain mutable consumer result fields. Each `ack_requirement_item` includes `minimum_visual_cue_type`, `minimum_visual_cue_requirement`, `allowed_consumer_types`, `hud_only_allowed`, `spatial_combat_fact`, `non_color_backup_required`, and `minimum_visible_duration_ms`.
...
**Given** `recovery_pause_count_per_round` exceeds 1 or 2 in one non-first round, **When** QA inspects runtime diagnostics, **Then** values above 1 produce a warning and values above 2 mark the Web experience test as failed unless an approved Runtime ADR or performance tuning change overrides the threshold. In the first round, any live-exchange recovery pause fails the first-round experience test.
...
**Given** `ack_deadline_ms` expires for a must-show fact, **When** visual ack is still missing, **Then** the responsible visual consumer or UI/HUD fallback must display the defined degraded visual cue from that fact's `ack_requirement_item`, append `ack_response_result = visually_degraded`, and only then may dependent automatic resume, AI/dummy decisions, or silent catch-up proceed. If the degraded cue also fails, runtime appends `ack_response_result = failed_terminal`, increments `combat_critical_ack_failure_count_per_round` when applicable, and blocks dependent automatic resume, AI/dummy decisions, or silent catch-up until stale discard, safe-neutral recovery, training replay, or restart policy resolves the failure.
...
**Given** QA runs the cold first-round scenario on the MVP blocking browser matrix, **When** any live combat exchange enters `recovery_pause`, visible `presentation_ack_wait`, terminal ack failure, `exchange_integrity = compromised_by_runtime_recovery`, `exchange_integrity = compromised_by_presentation_ack_wait`, or `exchange_integrity = compromised_by_ack_terminal_failure`, **Then** the first-round experience test fails, diagnostics record the compromised exchange reason, increment `interruption_epoch`, and normal match resume returns through safe neutral/reacquisition rather than granting a tactical read advantage inside the interrupted exchange.
...
The following criteria require explicit QA-facing instrumentation if it does not already exist: ordered event sequence keys, committed snapshot summaries, runtime state transition trace, backlog classification trace, rejected-command warnings, trace window bounds, command-source metadata for player/dummy/CPU command entry, UI runtime state payload, catch-up preflight descriptors/hashes/knowledge source/uncertainty flags, catch-up wall-clock/fairness guard decisions, catch-up stop reason precedence, event `presentation_tier`, committed-to-presented-running tick mapping, AI observation snapshots and hashes, AI fairness profile validation, interruption epochs, immutable payload diagnostics, atomic runtime presentation packets, per-fact ack requirement items, append-only ack response records, event delivery context, stale presentation diagnostics, runtime state sequence keys, Web performance budget metrics, MVP device class, cold first-round readiness metrics, compromised exchange counters, combat-critical ack failure counters, recovery/focus/interruption counters, and bounded trace payload diagnostics.
...
- VFX、音效、动画、相机震动和 HUD 闪烁可以延迟、压缩或去重表现，但不得改变、补写、删除或重排 authoritative combat facts；`normal_realtime_critical` facts 必须正常进入表现队列和显示水位顺序，不能当作可丢弃装饰；对于 `presentation_must_show`，required visual consumer 必须按 per-fact `ack_requirement_item` 达到 display watermark、明确视觉降级或 terminal failure resolution。normal-combat spatial facts 必须由 actor sprite、projectile VFX 或 combat VFX 满足；HUD/UI 只能辅助，除非模式明确是 training/debug/accessibility assist。audio-only ack 不足以恢复依赖该事实的 AI/补跑。
...
- UI runtime state payload 必须表达 `runtime_state`、`state_reason`、适用时的 `pause_reason`、适用时的 `focus_loss_reason`、适用时的 `audio_degradation_reason`、`previous_runtime_state` 或 `resume_target_state`、`runtime_state_sequence_key`、`requires_player_confirm`、`combat_input_policy`、`held_input_cleanup_required`、`held_input_cleanup_state`、`countdown_phase`、`countdown_visual_ack_id` when applicable、`player_facing_recovery_label`、`safe_resume_countdown_id`、`resume_confirm_consumed_by_ui`、`exchange_integrity` when applicable、`backlog_classification`、适用时的 `catch_up_stop_reason`、`presentation_ack_required`、`presentation_ack_id`、`presentation_ack_wait_reason` when applicable、`display_watermark_target`、适用时的 `ack_deadline_ms`、适用时的 `ack_wait_duration_ms`、`latest_ack_response_result` when applicable、`ack_deadline_missed_by_host_stall` when applicable、`round_instance_id` 和 `round_instance_sequence`。
